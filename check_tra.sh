...
usage() {
    cat <<EOF
Usage:
    $0 [--detailed] <output_dir> <task_range>

Examples:
    $0 /lustre/cbm/users/\${USER}/mc/out/tra "1-20"
    $0 --detailed /lustre/cbm/users/\${USER}/mc/out/tra "1-20"
    $0 --detailed /lustre/cbm/users/\${USER}/mc/out/tra "1,3,7-10"
...
if [[ ! -d "${BASE_DIR}" ]]; then
    printf "${RED}ERROR:${RESET} Transport output directory does not exist: %s\n" \
        "${BASE_DIR}"
    exit 1
fi
...
for task_id in "${TASK_IDS[@]}"; do
    log_file="${BASE_DIR}/${task_id}/transport.log.gz"
    ...
    if zgrep -Fq "${SUCCESS_PATTERN}" "${log_file}"; then
        ...
        failed_jobs+=("${task_id}|TRANSPORT DID NOT FINISH|${log_file}")
    fi
done
...
printf "${BOLD}                 TRANSPORT JOB SUMMARY${RESET}\n"
printf "${BOLD}${CYAN}============================================================${RESET}\n"
...
if ${DETAILED}; then
    ...
    printf '\n'
    printf "${BOLD}TRANSPORT JOB DETAILS${RESET}\n"
    printf '%s\n' "------------------------------------------------------------"
    ...
    if (( failed > 0 )); then
        ...
        for entry in "${failed_jobs[@]}"; do
            ...
            printf "  Task %-6s  ${RED}FAILED${RESET}  (%s)\n" \
                "${task_id}" "${reason}"
            printf "      %s\n" "${log_file}"
        done
    else
